...
class AggregatorService:
    ...
    def unread_comic_strips(self, comic):
        ...
        unreads = self.unread_strips().filter(comic__id=comic.id)
        return [unread.strip for unread in unreads]
    ...
    def unread_comic_strips_count(self, comic):
        ...
        return self.unread_strips().filter(comic__id=comic.id).count()

    def unread_strips_count(self):
        """Total number of unread strips."""
        return self.unread_strips().count()
