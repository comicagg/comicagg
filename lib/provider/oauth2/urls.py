...
app_name = "oauth2"
urlpatterns = [
    re_path(r"^authorize/?$", login_required(Capture.as_view()), name="capture"),
    re_path(
        r"^authorize/confirm/?$", login_required(Authorize.as_view()), name="authorize"
    ),
    re_path(r"^redirect/?$", login_required(Redirect.as_view()), name="redirect"),
    re_path(
        r"^access_token/?$", csrf_exempt(AccessTokenView.as_view()), name="access_token"
    ),
]
