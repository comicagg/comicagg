  ...
  if request.POST:
    ...
    if form.is_valid():
      ...
      if password != password2:
        form.errors.others['passwords_dont_match'] = True
      if captcha != 'comic':
        form.errors.others['captcha_error'] = True
      if re.compile('[^a-zA-Z0-9_]').match(username):
        form.errors.others['username_error'] = True
      if not form.errors.others:
        try:
          u = User.objects.create_user(username, email, password)
          return HttpResponseRedirect(reverse('done', args=['register']))
        except IntegrityError:
          form.errors.others['duplicated_user'] = True
